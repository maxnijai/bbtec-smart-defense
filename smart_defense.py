import os, json, time, base64, hmac, hashlib, re, threading
...
CACHE_TTL_USERS = int(os.environ.get('SMART_DEFENSE_CACHE_USERS_SEC', '900'))
CACHE_TTL_TICKETS = int(os.environ.get('SMART_DEFENSE_CACHE_TICKETS_SEC', '600'))
CACHE_TTL_AUDIT = int(os.environ.get('SMART_DEFENSE_CACHE_AUDIT_SEC', '300'))
CACHE_STALE_TICKETS = int(os.environ.get('SMART_DEFENSE_STALE_TICKETS_SEC', '7200'))
CACHE_STALE_USERS = int(os.environ.get('SMART_DEFENSE_STALE_USERS_SEC', '3600'))
MIN_SHEET_READ_INTERVAL = int(os.environ.get('SMART_DEFENSE_MIN_READ_INTERVAL_SEC', '90'))
DISK_CACHE_PATH = os.environ.get('SMART_DEFENSE_DISK_CACHE_PATH', '/tmp/bbtec_smart_defense_cache.json')
...
_SD_CACHE = {
    ...
    'last_read_attempt': {'ts': 0, 'data': None},
    'last_error': {'ts': 0, 'data': None},
}
_CACHE_LOCK = threading.RLock()
_SHEET_READ_LOCK = threading.Lock()

def _set_last_error(err):
    _SD_CACHE['last_error'] = {'ts': time.time(), 'data': str(err)}
    ...
    try:
        ...
        if os.path.exists(DISK_CACHE_PATH):
            try:
                ...
                allc = {}
        allc[name] = {'ts': time.time(), 'data': data}
...
def _cache_get(name, ttl, allow_disk=False):
    with _CACHE_LOCK:
        if _cache_valid(name, ttl):
            return _SD_CACHE[name]['data']
    if allow_disk:
        item = _disk_load(name)
...
def _cache_get_stale(name, stale_ttl, allow_disk=True):
    with _CACHE_LOCK:
        c = _SD_CACHE.get(name) or {}
        if c.get('data') is not None and (time.time() - c.get('ts', 0)) < stale_ttl:
            return c.get('data')
    if allow_disk:
        item = _disk_load(name)
        if item and item.get('data') is not None and (time.time() - item.get('ts', 0)) < stale_ttl:
            with _CACHE_LOCK:
                _SD_CACHE[name] = item
            return item.get('data')
    return None

def _cache_set(name, data, disk=False):
    with _CACHE_LOCK:
...
def load_users(get_client, force=False):
    if not force:
        cached = _cache_get('users', CACHE_TTL_USERS, allow_disk=False)
        if cached is not None:
            return cached
        stale = _cache_get_stale('users', CACHE_STALE_USERS, allow_disk=False)
        if stale is not None:
            return stale
    try:
        ws = open_ws(get_client, USER_SHEET, True)
        records = ws.get_all_records()
        _cache_set('users', records, disk=False)
        return records
...
def get_rows(get_client, force=False, allow_stale=True):
    """Ultra quota-safe ticket loader.
    - Normal page loads use memory/disk cache and do not hit Google Sheets repeatedly.
    - If Google Sheets returns 429, stale cache is served instead of breaking the page.
    - A minimum read interval prevents parallel browser requests from burning quota.
    """
    if not force:
        cached = _cache_get('tickets', CACHE_TTL_TICKETS, allow_disk=True)
        if cached is not None:
            headers, m, data = cached
            return None, headers, m, data
        stale = _cache_get_stale('tickets', CACHE_STALE_TICKETS, allow_disk=True)
        if stale is not None:
            headers, m, data = stale
            return None, headers, m, data

    now_ts = time.time()
    last_attempt = (_SD_CACHE.get('last_read_attempt') or {}).get('ts', 0)
    if not force and (now_ts - last_attempt) < MIN_SHEET_READ_INTERVAL:
        stale = _cache_get_stale('tickets', CACHE_STALE_TICKETS, allow_disk=True)
        if stale is not None:
            headers, m, data = stale
            return None, headers, m, data

    acquired = _SHEET_READ_LOCK.acquire(blocking=False)
    if not acquired:
        stale = _cache_get_stale('tickets', CACHE_STALE_TICKETS, allow_disk=True)
        if stale is not None:
            headers, m, data = stale
            return None, headers, m, data
        if _SHEET_READ_LOCK.acquire(timeout=12):
            _SHEET_READ_LOCK.release()
        cached = _cache_get('tickets', CACHE_STALE_TICKETS, allow_disk=True)
        if cached is not None:
            headers, m, data = cached
            return None, headers, m, data
        _SHEET_READ_LOCK.acquire()
        acquired = True

    try:
        _SD_CACHE['last_read_attempt'] = {'ts': time.time(), 'data': True}
        ...
        payload = (headers, m, data)
        _cache_set('tickets', payload, disk=True)
...
def register_smart_defense(app, get_client, log):
    ...
    def sd_cache_status():
        ...
        return jsonify({'ok': True, 'cache': {
            ...
            'stale_tickets_sec': CACHE_STALE_TICKETS,
            'min_read_interval_sec': MIN_SHEET_READ_INTERVAL,
            'last_error': _SD_CACHE.get('last_error', {}).get('data'),
            'last_error_age_sec': round(time.time() - _SD_CACHE.get('last_error', {}).get('ts', 0), 1) if _SD_CACHE.get('last_error', {}).get('data') else None,
            'ticket_count_cached': len((_SD_CACHE.get('tickets', {}).get('data') or [None, None, []])[2]) if _SD_CACHE.get('tickets', {}).get('data') is not None else 0
        }, 'updated_at': now()})


    @app.route('/api/smart-defense/debug')
    def sd_debug():
        user, err = auth()
        if err: return err
        try:
            _, headers, _, data = get_rows(get_client, force=False)
            visible = [r for r in data if can_view(user, r.get('region'), r.get('province'), r.get('owner_group'))]
            owners = sorted(list({r.get('owner_group','') for r in data if r.get('owner_group')}))[:50]
            provinces = sorted(list({r.get('province','') for r in data if r.get('province')}))[:50]
            return jsonify({'ok': True, 'version': 'ultra_quota_safe_2026_04_27', 'sheet_id': SHEET_ID, 'ticket_sheet': TICKET_SHEET, 'user_sheet': USER_SHEET, 'headers_sample': headers[:50], 'raw_count': len(data), 'visible_count': len(visible), 'sample_owner_groups': owners, 'sample_provinces': provinces, 'user': user, 'cache': {'tickets_cached': _SD_CACHE['tickets'].get('data') is not None, 'tickets_age_sec': round(time.time() - _SD_CACHE['tickets'].get('ts', 0), 1) if _SD_CACHE['tickets'].get('data') is not None else None, 'last_error': _SD_CACHE.get('last_error', {}).get('data')}})
        except Exception as e:
            log.exception('sd debug failed')
            return jsonify({'ok': False, 'error': str(e), 'sheet_id': SHEET_ID, 'ticket_sheet': TICKET_SHEET}), 500
